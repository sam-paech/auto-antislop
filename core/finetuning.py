...
def run_dpo_finetune(config: dict, experiment_run_dir: Path):
    ...
    if CALC_VAL_STATS:
        ...
        if True: # skip this check for now
            pre_train_rows, pre_train_stats = _gap_stats(model, train_ds.select(range(min(200, len(train_ds)))), collate, "train_pre", ref_model=None, use_null_ref=True)
            pre_val_rows , pre_val_stats   = _gap_stats(model, val_ds.select(range(min(200, len(val_ds)))),   collate, "val_pre", ref_model=None, use_null_ref=True)
    ...
    if CALC_VAL_STATS:
        ...
        # ────────────────────────────────────────────────────────────────────
        post_train_rows, post_train_stats = _gap_stats(model, train_ds.select(range(min(200, len(train_ds)))), collate, "train_post", ref_model=None, use_null_ref=False)
        post_val_rows , post_val_stats   = _gap_stats(model, val_ds.select(range(min(200, len(val_ds)))),   collate, "val_post", ref_model=None, use_null_ref=False)
